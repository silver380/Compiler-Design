%{
    #include <stdio.h>
%}

%token INT CHAR IF ELSE ELSEIF WHILE FOR RETURN VOID MAIN CONTINUE BREAK IDENTIFIER INT_CONST OUT_OF_RANGE
ONE_LINE_COMMENT MULTI_LINE_COMMENT LESS_EQUAL EQUAL NOT_EQUAL GREATER_EQUAL BITWISE_OR BITWISE_AND OR AND 
BITWISE_XOR NOT PLUS MINUS DIVISION MULTIPLY ASSIGN LEFTPAREN RIGHTPAREN LEFTBRACKET RIGHTBRACKET LEFTB
RIGHTB COMMA DOT MYSTERY CHAR_CONST LESS GREATER TPLUS TMINUS

%start input

%%
input:
    COMMENTS FUNCTIONS COMMENTS 
    ;
FUNCTIONS:
    TYPE IDENTIFIER LEFTPAREN INP RIGHTPAREN LEFTB STMTS RIGHTB  FUNCTIONS
    |
    INT MAIN LEFTPAREN RIGHTPAREN LEFTB STMTS RIGHTB
    ;
TYPE:
    INT
    |
    VOID
    |
    CHAR
    ;
INP:
    TYPE_AR IDENTIFIER 
    |
    TYPE_AR IDENTIFIER COMMA TYPE_AR IDENTIFIER 
    |
    TYPE_AR IDENTIFIER COMMA TYPE_AR IDENTIFIER COMMA TYPE_AR IDENTIFIER
    ;
INP2:
    ARGUMENTS
    |
    ARGUMENTS COMMA ARGUMENTS
    |
    ARGUMENTS COMMA ARGUMENTS COMMA ARGUMENTS
    ;
ARGUMENTS:
    IDENTIFIER
    |
    INT_CONST
    |
    CHAR_CONST
    ;
TYPE_AR:
    INT
    |
    CHAR
;
STMTS:
    STMT STMTS
    | STMT
    |
    ;
STMT:
    IF LEFTPAREN CONDITION RIGHTPAREN LEFTB STMTS RIGHTB ELSEIFGS ELSEG
    |
    WHILE LEFTPAREN CONDITION RIGHTPAREN LEFTB STMTS_LOOP RIGHTB
    |
    FOR LEFTPAREN EXPR COMMA CONDITION COMMA EXPR RIGHTPAREN LEFTB STMTS_LOOP  RIGHTB
    |
    EXPR DOT
    |
    RETURN DOT
    |
    RETURN EXPR DOT
    |
    COMMENTS
    |
    BREAK DOT
    |
    CONTINUE DOT
    ;
STMTS_LOOP:
    STMTS_LOOP  BREAK DOT STMTS_LOOP
    |
    STMTS
    |
    STMTS_LOOP CONTINUE DOT STMTS_LOOP
    /* IF LEFTPAREN CONDITION RIGHTPAREN LEFTB STMTS BREAK DOT STMTS RIGHTB ELSEIFGS2 ELSEG2
    |
    IF LEFTPAREN CONDITION RIGHTPAREN LEFTB STMTS CONTINUE DOT STMTS RIGHTB ELSEIFGS2 ELSEG2 */
    ;
ELSEIFGS:
        ELSEIF LEFTPAREN CONDITION RIGHTPAREN LEFTB STMTS RIGHTB ELSEIFGS
        |
    ;
/* ELSEIFGS2:
    ELSEIF LEFTPAREN CONDITION RIGHTPAREN LEFTB STMTS BREAK DOT STMTS RIGHTB ELSEIFGS
    |
    ELSEIF LEFTPAREN CONDITION RIGHTPAREN LEFTB STMTS CONTINUE DOT STMTS RIGHTB ELSEIFGS
    |
    ; */
ELSEG:
    ELSE LEFTB STMTS RIGHTB
    |
    ;
/* ELSEG2:
    ELSE LEFTB STMTS BREAK DOT STMTS RIGHTB
    |
    ELSE LEFTB STMTS CONTINUE DOT STMTS RIGHTB
    |
    ; */
EXPR:

    |
    INT IDENTIFIER
    |
    INT IDENTIFIER ASSIGN E
    |
    INT IDENTIFIER ASSIGN FUNCTION_CALL
    |
    CHAR IDENTIFIER
    |
    CHAR IDENTIFIER ASSIGN CHAR_CONST
    |
    CHAR IDENTIFIER ASSIGN FUNCTION_CALL
    |
    CHAR IDENTIFIER ASSIGN E
    |
    IDENTIFIER ASSIGN E
    |
    IDENTIFIER ASSIGN FUNCTION_CALL
    |
    IDENTIFIER ASSIGN CHAR_CONST
    |
    E
    |
    LEFTPAREN EXPR RIGHTPAREN
    |
    IDENTIFIER TPLUS
    |
    IDENTIFIER TMINUS
    |
    TPLUS IDENTIFIER
    |
    TMINUS IDENTIFIER
    ;

E:
    T EP 
    ;
EP:
    PLUS T EP
    |
    MINUS T EP
    |
    BITWISE_AND T EP
    |
    BITWISE_OR T EP
    |
    BITWISE_XOR T EP
    |
    ;
T:
    F TP 
    ;
TP:
    MULTIPLY F TP
    |
    DIVISION F TP
    |
    ;
F:
    LEFTPAREN E RIGHTPAREN
    |
    IDENTIFIER
    |
    INT_CONST
    |
    FUNCTION_CALL
    ;
CONDITION:
    EXPR COPERATOR CONDITION
    |
    EXPR
    |
    LEFTPAREN CONDITION RIGHTPAREN
    |
    NOT CONDITION
    ;
COPERATOR:
    LESS 
    | LESS_EQUAL 
    | EQUAL
    | NOT_EQUAL 
    | GREATER 
    | GREATER_EQUAL 
    | OR 
    | AND
    ;
COMMENTS:
    ONE_LINE_COMMENT
    |
    MULTI_LINE_COMMENT
    |
    ;
FUNCTION_CALL:
    IDENTIFIER LEFTPAREN INP2 RIGHTPAREN
%%
void yyerror(char *s){
    printf("%s\n",s );

}
int yywrap(){
    
    return 1;

}

int main(void){
    return yyparse();
}